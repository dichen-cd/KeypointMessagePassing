...
from .datasets import init_pose_dataset
from ..utils.tools import collate_torch_geo
...
class ImageDataManager(DataManager):
    ...
    def __init__(
        ...
        if load_train_targets:
            ...
            self.train_loader_t = PrefetchDataLoader(
                trainset_t,
                sampler=build_train_sampler(
                    trainset_t.train,
                    train_sampler_t,
        ...
        for name in self.targets:
            ...
            self.test_loader[name]['query'] = PrefetchDataLoader(
                queryset,
            ...
            self.test_loader[name]['gallery'] = PrefetchDataLoader(
                galleryset,
                batch_size=batch_size_test,
                shuffle=False,
                num_workers=workers,
...
class PoseDataManager(DataManager):
    ...
    def __init__(
        ...
        super(PoseDataManager, self).__init__(
            sources=sources,
            targets=targets,
            height=height,
            width=width,
        ...
        for name in self.sources:
            trainset_ = init_pose_dataset(
                ...
                headless=headless,
                rm_glitches=rm_glitches,
                include_dynamics=include_dynamics,
                include_spatial_links=include_spatial_links,
                include_temporal_links=include_temporal_links
            )
            trainset.append(trainset_)
        trainset = sum(trainset)
